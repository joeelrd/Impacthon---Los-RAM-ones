...
export default function ChatbotPanel({ context }: ChatbotPanelProps) {
  ...
  const overrideComponents = {
    ...
    code: ({node, inline, ...props}: any) => (
      inline ? 
      <code style={{ background: 'rgba(0,0,0,0.3)', padding: '2px 6px', borderRadius: '4px', fontFamily: 'monospace', fontSize: '0.85em', color: '#ff7d45' }} {...props} /> :
      <pre style={{ background: 'rgba(0,0,0,0.4)', padding: '12px', borderRadius: '8px', overflowX: 'auto', marginBottom: '10px', fontSize: '0.9em' }}>
    ...
    h2: ({node, ...props}: any) => <h2 style={{ fontSize: '1.2em', marginBottom: '10px', color: '#e0e0f0' }} {...props} />,
    h3: ({node, ...props}: any) => <h3 style={{ fontSize: '1.1em', marginBottom: '8px', color: '#e0e0f0' }} {...props} />,
    table: ({node, ...props}: any) => <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '10px' }} {...props} /></div>,
    th: ({node, ...props}: any) => <th style={{ borderBottom: '1px solid rgba(255,255,255,0.2)', padding: '8px', textAlign: 'left', color: 'var(--accent-cyan)' }} {...props} />,
    td: ({node, ...props}: any) => <td style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', padding: '8px' }} {...props} />
  };
  ...
  return (
    <div style={{
      ...
      boxShadow: '0 8px 32px rgba(0, 0, 0, 0.4), inset 0 0 16px rgba(101, 203, 243, 0.05)',
      ...
      <div style={{
        ...
        <div>
          <h3 style={{ margin: 0, fontSize: '1rem', color: '#fff', fontWeight: 600, letterSpacing: '0.5px' }}>Gemini IA</h3>
          ...
          <div key={msg.id} style={{
            ...
            <div style={{
              ...
              background: msg.sender === 'user' 
                ...
                : 'var(--bg-color-main)',
              border: '1px solid',
              borderColor: msg.sender === 'user' ? 'rgba(101,203,243,0.3)' : 'rgba(255,255,255,0.05)',
        ...
        <form onSubmit={handleSend} style={{ display: 'flex', gap: '12px' }}>
          ...
          <button
            ...
            style={{
              ...
              height: '46px',
              borderRadius: '50%',
              background: input.trim() && !isLoading ? 'linear-gradient(135deg, var(--accent-cyan), #0053d6)' : 'rgba(255,255,255,0.1)',
              border: 'none',
              display: 'flex',
              alignItems: 'center',
